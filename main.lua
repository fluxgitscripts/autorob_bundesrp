...
-- Koordinaten (Global definiert für Bot & Erkennung)
...
-- Hintergrundschleife für die Polizei-Erkennung
task.spawn(function()
    while true do
        task.wait(0.3)
                ...
                if otherPlayer ~= player and otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    if getCustomJobName(otherPlayer) == "Police" then
                        ...
                        if distance <= 50 then
                            ...
                            -- NEU: Erst schnell zur Serverhop-Position tweenen (0.5s Dauer)
                            ...
                            task.wait(0.3) -- Kurze Sicherheitspause nach dem Ankommen
                            performServerHop() -- Jetzt erst den Serverhop ausführen
                            break
                        end
                    end
                end
...
local function botLoop()
    while botRunning do
        ...
        -- LOOT EINSMAMMELN (Umsortiert auf schnellen Tween)
        if itemToCollect and currentId then
            foundSomething = true
            local itemPos = getItemPosition(itemToCollect)
            if itemPos then
                -- NEU: Schneller Tween statt Instant-Teleport (0.18 Sekunden)
        ...
        -- MELEE ATTACK / LOOT KNACKEN (Umsortiert auf schnellen Tween)
            ...
            if itemPos then
                ...
                if hitbox and hitbox.Parent then
                    ...
                    pcall(function()
                        ...
                        -- NEU: Schneller Richtungs- und Positions-Tween (0.15 Sekunden)
